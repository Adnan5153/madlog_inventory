...
@section('content')
    ...
    <x-admin.page-header :title="$adjustment->reference">
        <x-slot:actions>
            @if(($adjustment->status instanceof LubricantStockAdjustmentStatus) && $adjustment->status === LubricantStockAdjustmentStatus::Pending && auth()->user()?->isAdmin())
                <form method="POST" action="{{ route('admin.lubricant-stock-adjustments.approve', $adjustment) }}" class="d-inline">
                    @csrf
                    <button class="btn btn-success"><i class="bi bi-check2 me-1"></i> Approve</button>
                </form>
                <form method="POST" action="{{ route('admin.lubricant-stock-adjustments.reject', $adjustment) }}" class="d-inline"
    ...
    <div class="admin-card">
        <dl class="row mb-0">
            ...
            <dd class="col-9">
                @php $st = $adjustment->status instanceof LubricantStockAdjustmentStatus ? $adjustment->status : null; @endphp
            ...
            <table class="table table-sm align-middle mb-0">
                ...
                <tbody>
                    @foreach($adjustment->items as $item)
                        <tr>
                            <td>
                                <a href="{{ route('admin.lubricants.show', $item->lubricant) }}" class="text-decoration-none">
                                    {{ $item->lubricant?->name ?? '—' }}
                                </a>
                                @if($item->lubricant?->lubricant_code)
                                    <div class="text-muted small">{{ $item->lubricant->lubricant_code }}</div>
                                @endif
                            </td>
                            <td>{{ $item->bin?->code ?? '—' }}</td>
